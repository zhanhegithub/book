...
<jsp:include page="header.jsp">
	<jsp:param name="msg" value="添加图书"/>
		...
		<div id="main">
			<form action="BookServlet?action=add" enctype="multipart/form-data" method="post" onsubmit="return check();">
				<table>
					<tr>
						<td>名称</td>
...
<script>
	...
	function check(){
		...
		var salesobj = document.getElementById("sales");
		var stockobj = document.getElementById("stock");
		salesvalue = salesobj.value;
		pricevalue = priceobj.value;
		stockvalue = stockobj.value;
		var s1 = /^[0-9]*$/;
		var s2 = /^[\+\-]?\d*?\.?\d*?$/;
		if(!s2.test(pricevalue)){
			alert("价格为整数或小数");
			return false;
		}
		if(!s1.test(salesvalue)){
			alert("销量为整数");
			return false;
		}
		if(!s1.test(stockvalue)){
			alert("库存为整数");
			return false;
		}
		return true;
